/**
 * ConfigFormFields — 子区域组件集合
 * 从 ConfigForm.tsx 拆分，包含所有可折叠子区域和字段原语
 */
...
export function NumberInput(props: React.InputHTMLAttributes<HTMLInputElement> & { width?: string }) {
...
export const VALIDATION_STEPS = [
  { id: 'V0', label: 'V0 · PROJECT_DETECT', desc: '检测语言、框架、测试工具、入口点（始终自动执行，不可跳过）', alwaysOn: true },
  { id: 'V1', label: 'V1 · DEPENDENCY',     desc: '安装项目依赖',                               alwaysOn: false },
  { id: 'V2', label: 'V2 · STATIC_ANALYSIS',desc: 'Linting / 类型检查',                         alwaysOn: false },
  { id: 'V3', label: 'V3 · UNIT_TEST',       desc: '运行单测；覆盖率低于 target_coverage 时自动生成测试', alwaysOn: false },
  { id: 'V4', label: 'V4 · BUILD_VERIFY',    desc: '构建 / 安装验证',                            alwaysOn: false },
  { id: 'V5', label: 'V5 · SMOKE_TEST',      desc: '入口点冒烟测试',                             alwaysOn: false },
  { id: 'V6', label: 'V6 · REPORT',          desc: '生成验证报告（有错误时生成；全部成功则进入 CI/CD 流程，不生成报告）', alwaysOn: false },
] as const
